...
`siteName`, `language`, `direction`, `baseUrl` (`"https://drgilamd.com"`), `aboutUrl` (currently `""` — the About page doesn't exist yet), `header` (doctor's name/role), `socialLinks`, `uiLabels`, `homepage` (photo, welcome paragraphs, `topicSelectorLabel`/`topicSelectorPlaceholder`), `topicsIndexPage`, `medicalDisclaimerPage`, `allTopicsButton` (top-level `{text, url}` — the "לכל הנושאים באתר" button, shared as-is by the homepage, every Node page's second nav button, and the Medical Disclaimer page's own bottom button, since they are literally the same button in three places, just rendered with each page's own existing button styling), `moreLinkButton` (Node pages' own FIRST nav button — `{text}` only, no stored `url`: its href is a dynamic per-Node `/topics/{id}/` computed at build time from the Node's own first public category, see Topic Navigation section below), `homeNavBar`.
...
* **Topic-context navigation (browser-side only).** When a Topic page renders its Node cards (`render_node_card_html(node, from_topic_id=topic["id"])`), each card's `href` carries a `?fromTopic={topicId}` query param — throwaway, per-URL context, never `localStorage`, never sent anywhere. On the Node page, a small inline script in `template.html` (`applyTopicNavContext()`) reads that param, whitelist-validates it (`^[a-z0-9-]+$`), and if valid, rewrites the first pink button's `href` (found via its `data-topic-nav="primary"` attribute — the second, always-static button also uses `.more-link` and must not be touched) to `/topics/{fromTopic}/`, so a visitor who arrived from a specific Topic page returns to that same Topic rather than always landing on the Node's first public category. Direct/external/AI-search/social visits carry no param, so the button keeps `_first_public_topic_id()`'s server-rendered fallback unchanged. This is purely a client-side rewrite of an already-correct server-rendered default — it never affects canonical URLs, `og:url`, JSON-LD, `sitemap.xml`, or `robots.txt` (all built server-side from clean paths, confirmed independent of any query string), and Cloudflare Pages serves the identical static file regardless of the query string, so there's no extra build output. The Template Node never renders the `data-topic-nav="primary"` button at all (no public category), so the script simply has nothing to find and no-ops, with or without the param.
